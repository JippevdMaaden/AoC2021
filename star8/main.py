...
def main():
    ...
    for i, string_number in enumerate(bingo_input_list):
        for bingo_board in bingo_board_list:
            bingo_board.mark_number(string_number=string_number)

            # Our list contains an empty board, and im cant be bother to fix the parsing
            # So we check if len ==2 rather than 1
            if len(bingo_board_list) == 2 and bingo_board.check_for_win():
                unmarked_sum = 0
        ...
        bingo_board_list = [board for board in bingo_board_list if not board.check_for_win()]
